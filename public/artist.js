const Artist={
    init(){
        ...
        <div id="artist-modal" class="fixed inset-0 bg-[#121212] flex flex-col z-[150]" style="display:none;">
            <div class="flex items-center gap-3 p-4 pt-6">
                <button onclick="Artist.close()" class="text-white p-3 hover:bg-white/10 rounded-full active:scale-90"><i data-lucide="x" class="w-6 h-6"></i></button>
            ...
            if(d.status&&d.result){
                ...
                if(a.topSongs&&a.topSongs.length>0){
                    ...
                    a.topSongs.slice(0,10).forEach(function(s,i){
                        ...
                        html+=`
                        <div onclick="Artist.play('${s.videoId}','${es(s.title).replace(/'/g,"\\'")}','${es(s.artist||a.name).replace(/'/g,"\\'")}')" class="flex items-center gap-3 p-3 hover:bg-[#282828] rounded-xl cursor-pointer active:scale-[0.98] transition-all group">
                            ...
                            <div class="flex-1 truncate">
                                <p class="font-medium text-sm truncate group-hover:text-[#1ed760] transition-colors">${es(s.title)}</p>
                                <p class="text-[#6b7280] text-xs truncate">${es(s.artist||a.name)}</p>
                            </div>
                            <i data-lucide="play" class="w-5 h-5 text-[#6b7280] opacity-0 group-hover:opacity-100 group-hover:text-[#1ed760] transition-all"></i>
                        </div>`;
                    });
                    html+='</div></div>';
                }
                ...
                if(a.topAlbums&&a.topAlbums.length>0){
                    ...
                    a.topAlbums.forEach(function(al){
                        ...
                        <div onclick="Artist.open('${al.browseId}','${es(al.name).replace(/'/g,"\\'")}')" class="flex-shrink-0 w-36 cursor-pointer group">
                            ...
                            <p class="font-medium text-sm truncate group-hover:text-[#1ed760] transition-colors">${es(al.name)}</p>
                ...
                if(a.topSingles&&a.topSingles.length>0){
                    ...
                    a.topSingles.forEach(function(sg){
                        ...
                        <div class="flex-shrink-0 w-36 cursor-pointer group">
                            ...
                            <p class="font-medium text-sm truncate group-hover:text-[#1ed760] transition-colors">${es(sg.name)}</p>
                ...
                if(a.topVideos&&a.topVideos.length>0){
                    ...
                    a.topVideos.forEach(function(vd){
                        ...
                        <div onclick="Artist.play('${vd.videoId||''}','${es(vd.name).replace(/'/g,"\\'")}','${es(vd.artist||a.name).replace(/'/g,"\\'")}')" class="flex-shrink-0 w-44 cursor-pointer group">
                            ...
                            <p class="font-medium text-sm truncate group-hover:text-[#1ed760] transition-colors">${es(vd.name)}</p>
                ...
                if(a.similarArtists&&a.similarArtists.length>0){
                    ...
                    a.similarArtists.forEach(function(s){
                        ...
                        <div onclick="Artist.open('${s.browseId}','${es(s.name).replace(/'/g,"\\'")}')" class="flex-shrink-0 text-center cursor-pointer group">
                            <div class="w-24 h-24 rounded-full overflow-hidden shadow-lg ring-2 ring-transparent group-hover:ring-[#1ed760]/50 transition-all">
                                <img src="${im}" class="w-full h-full object-cover" onerror="this.src='${FI}'" />
                            </div>
                            <p class="text-xs mt-2 truncate w-24 group-hover:text-[#1ed760] transition-colors">${es(s.name)}</p>
                        </div>`;
                    });
                    html+='</div></div>';
                }
    ...
    play(vid,title,artist){
        ...
        S.ps='artist';S.pl=[S.ct];S.pi=0;S.lyricsLoaded=false;S.ld={type:'none',lines:[]};S.cli=-1;UU();MP.show();S.il=true;UB();
