const Home={
    ...
    async fetch(){
        try{
            var q=['lagu viral indonesia 2026','top hits indonesia','lagu terbaru'][Math.floor(Math.random()*3)];
    ...
    show(){
        ...
        s.innerHTML=S.ht.slice(6,12).map(function(t,i){
            return '<div onclick="PK(\'home2\','+i+')" class="flex-shrink-0 w-40 cursor-pointer active:scale-95" style="animation-delay:'+((i+6)*50)+'ms"><div class="w-40 h-40 mb-2 relative"><img src="'+t.cover+'" class="w-full h-full object-cover rounded-xl shadow-lg" onerror="this.src=\''+FI+'\'" /><div class="absolute bottom-2 right-2 bg-[#1ed760] text-black rounded-full p-3 opacity-0 hover:opacity-100 transition-all"><i data-lucide="play" class="w-5 h-5 fill-current ml-0.5"></i></div></div><h3 class="font-semibold text-sm truncate">'+es(t.title)+'</h3><p class="text-[#6b7280] text-xs truncate mt-1">'+es(t.artist)+'</p></div>';
        }).join('');
        lucide.createIcons();
    },
