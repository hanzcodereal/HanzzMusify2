const App={
    init(){
        ...
        <div class="pt-12 px-4 text-center">
            ...
            <h1 class="text-3xl font-black bg-gradient-to-r from-[#1ed760] to-white bg-clip-text text-transparent mb-1">HanzzMusify</h1>
            <p class="text-[#b3b3b3] text-sm mb-6">Streaming Musik YouTube dengan Lirik</p>
            
            <div class="bg-[#1a1a1a] rounded-2xl p-5 max-w-sm mx-auto border border-white/10 space-y-3 text-left mb-4">
            ...
            <div class="bg-[#1a1a1a] rounded-2xl p-5 max-w-sm mx-auto border border-white/10 space-y-3 text-left mb-6">
                ...
                <div class="flex justify-between"><span class="text-[#6b7280] text-sm">Email</span><span class="text-[#1ed760] font-bold text-sm">hanzzcodee@gmail.com</span></div>
            </div>
        </div>`;
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            ...
            setTimeout(function(){FullPlayer.open();S.il=true;UB();if(S.yp&&S.yr)S.yp.loadVideoById({videoId:videoId});},400);
        }).catch(function(){
    ...
    showSharePopup(videoId){
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','HanzzMusify',FI);});
    },
    ...
    switch(t){
        ...
        ['home','search','library','dev'].forEach(function(n){var b=gid('nav-'+n);if(!b)return;b.classList.remove('text-[#1ed760]');b.classList.add('text-[#6b7280]');var i=b.querySelector('i');if(i)i.classList.remove('fill-current');});
        var ab=gid('nav-'+t);if(!ab)return;ab.classList.remove('text-[#6b7280]');ab.classList.add('text-[#1ed760]');if(t==='home')ab.querySelector('i').classList.add('fill-current');
...
const Library={
    ...
    playSong(plId,index){var pls=getUserPlaylists();var pl=pls.find(function(p){return p.id===plId;});if(!pl||!pl.songs[index])return;S.pl=pl.songs;S.pi=index;S.ps='playlist';S.ct=S.pl[S.pi];UU();MP.show();S.il=true;UB();if(S.yp&&S.yr&&S.ct.videoId)S.yp.loadVideoById({videoId:S.ct.videoId});}
};