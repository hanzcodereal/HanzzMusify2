...
const S={ht:[],sr:[],ar:[],sq:'',filter:'all',ct:null,pl:[],pi:-1,ps:'',ip:false,il:false,rm:'off',autoNext:true,yp:null,yr:false,iv:null,pt:0,pd:0,at:'home',ld:{type:'none',lines:[]},cli:-1,lo:false,lyricsLoaded:false,server:'1'};
...
function SK(v){
    if(S.yp && S.yr && S.pd>0) {
        S.yp.seekTo((parseFloat(v)/100)*S.pd,true);
    }
}
...
async function FL(vid){
    if(S.lyricsLoaded && S.ct && S.ct.videoId === vid && S.ld.lines.length > 0) return;
    var c=gid('lyrics-content');
    if(!c) return;
    c.innerHTML = '<div class="flex justify-center items-center h-full"><div class="w-8 h-8 border-3 border-[#1ed760] border-t-transparent rounded-full animate-spin"></div><p class="text-[#6b7280] ml-3 text-sm">Memuat lirik...</p></div>';
    S.ld={type:'none',lines:[]};
    S.cli=-1;
    try{
        var r=await fetch(API.lyrics+'?id='+vid);
        var d=await r.json();
        if(d.status && d.result.lyrics && d.result.lyrics.lines.length>0){
            ...
            S.ld.lines.forEach(function(li,i){
                var timeStr = li.time ? fm(li.time) : '0:00';
            ...
            c.innerHTML='<div class="flex justify-center items-center h-full"><i data-lucide="music" class="w-16 h-16 opacity-30"></i><p class="text-[#6b7280] ml-3">Lirik tidak tersedia</p></div>';
            lucide.createIcons();
        }
    }catch(er){
        c.innerHTML='<div class="flex justify-center items-center h-full"><i data-lucide="music" class="w-16 h-16 opacity-30"></i><p class="text-[#6b7280] ml-3">Lirik tidak tersedia</p></div>';
