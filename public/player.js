...
const S={ht:[],sr:[],ar:[],sq:'',filter:'all',ct:null,pl:[],pi:-1,ps:'',ip:false,il:false,rm:'off',autoNext:true,yp:null,yr:false,iv:null,pt:0,pd:0,at:'home',ld:{type:'none',lines:[]},cli:-1,lo:false,lyricsLoaded:false};
...
async function FL(vid){
    if(S.lyricsLoaded && S.ct && S.ct.videoId === vid) return;
    ...
    try{
        ...
        if(d.status && d.result.lyrics && d.result.lyrics.lines.length>0){
            ...
            S.ld.lines.forEach(function(li,i){
                ...
                html+='<p class="lyric-line text-left px-2" data-time="'+li.time+'" data-index="'+i+'" onclick="SLT('+li.time+')" style="color:#6b7280;font-size:0.85rem;line-height:2;transition:all 0.3s ease;">'+es(li.text)+'</p>';
            });
...
function toggleLyrics(){
    ...
    if(S.lo){
        ...
        if(S.ct && S.ct.videoId){
            if(!S.lyricsLoaded || S.ld.lines.length===0){
                FL(S.ct.videoId);
            } else {
                var l=gid('lyrics-loading'),c=gid('lyrics-content'),e=gid('lyrics-empty');
